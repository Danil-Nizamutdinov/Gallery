...
export const setPaintings = (paintings) => ({type: 'SET_PAITINGS', paintings})
...
const toggleButtonPreviousFalse = () => ({type: 'TOGGLE_BUTTON_PREVIOUS_FALSE'})
const toggleButtonPreviousTrue = () => ({type: 'TOGGLE_BUTTON_PREVIOUS_TRUE'})
...
export const getPaintingsSearch = (authorId,locationId,bodyText) => {
    return (dispatch) => {
        searchAPI.searchPaintings(authorId,locationId, bodyText).then( response => {
            dispatch(setPaintings(response))
        })
    }
}
